// fork of ember-i18n by dierbro : https://github.com/dierbro/ember-i18n
(function(window) {
    ...
    findTemplate = function(key, setOnMissing) {
        ...
        var translations = Ember.I18n.get("translations");
        var result = lookupKey(key, translations);

        if (setOnMissing) {
            if (result == null) {
                result = translations[key] = function() { return "Missing translation: " + key; };
                result._isMissing = true;
                warn("Missing translation: " + key);
                Ember.I18n[(typeof Ember.I18n.trigger === 'function' ? 'trigger' : 'fire')]('missing', key); //Support 0.9 style .fire
            }
        }

        if ((result != null) && !jQuery.isFunction(result)) {
            result = translations[key] = Ember.I18n.compile(result);
        }
    ...
    keyExists = function(key) {
        var translation = lookupKey(key, Ember.I18n.get("translations"));
    ...
    I18n = Ember.Object.extend(Ember.Evented, {
        ...
        t: function(key, context) {
            ...
            template = this.template(key, context.count);
    ...
    Handlebars.registerHelper('t', function(key, options) {
        var attrs, context, data, elementID, result, tagName, view, observer2, invoker2;
        context = this;
        attrs = options.hash;
        data = options.data;
        view = data.view;
        ...
        Em.keys(attrs).forEach(function(property) {
            ...
            if (isBindingMatch) {
                ...
                observer = function() {
                    ...
                    return elem.html(Ember.I18n.t(key, attrs));
                };
        ...
        if(Ember.I18n.get('bindTranslations')){
            var observer, invoker;

            observer = function(){
                var elem;
                if (view.get('state') !== 'inDOM') {
                    Ember.I18n.removeObserver("translations", context, invoker);
                    return;
                }
                elem = view.$("#" + elementID);
                return elem.html(Ember.I18n.t(key, attrs));
            ...
            Ember.I18n.addObserver("translations", context, invoker);
        }
        result = '<%@ id="%@">%@</%@>'.fmt(tagName, elementID, Ember.I18n.t(key, attrs), tagName);
    ...
    Handlebars.registerHelper('translateAttr', function(options) {
        ...
        Em.keys(attrs).forEach(function(property) {
            var translatedValue;
            translatedValue = Ember.I18n.t(attrs[property]);
